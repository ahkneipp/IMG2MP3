...
def get_intensity(img, mask):
    hsv_img = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
    # hsv_img = np.multiply(hsv_img, np.expand_dims(masks[::, ::, i], axis=2))
    hist = cv2.calcHist([hsv_img], channels=[2], mask=mask, histSize=[2], ranges=[0, 255])
    return np.argmax(hist)
...
def get_saturation(img, mask):
    hsv_img = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
    # hsv_img = np.multiply(hsv_img, np.expand_dims(masks[::, ::, i], axis=2))
    hist = cv2.calcHist([hsv_img], channels=[1], mask=mask, histSize=[4], ranges=[0, 255])
    return np.argmax(hist)
...
def get_noisiness(img, mask):
    ...
    return int(np.average(output) * 24/43.0)
...
ImgData = collections.namedtuple('ImgData', 'size, color, intensity, saturation, noise')

def get_data(img, masks):
    print("Processing Countours", end="", flush=True)
    data = []
    height, width, num_masks = np.shape(masks)
    for i in range(num_masks):
        print(".", end="", flush=True)
        mask = masks[:, :, i]
        data.append(ImgData(size=get_size(mask),
                            color=get_colors(img, mask),
                            intensity=get_intensity(img, mask),
                            saturation=get_saturation(img, mask),
                            noise=get_noisiness(img,mask)))
    return data
